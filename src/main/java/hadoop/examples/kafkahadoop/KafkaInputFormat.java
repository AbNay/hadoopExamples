...
public class KafkaInputFormat extends InputFormat<LongWritable, BytesWritable> {
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException, InterruptedException {
		...
		ZkUtils zk = new ZkUtils(conf.get("kafka.zk.connect"), 
		...
		for (String topic : inputTopics) {
			...
			for (String partition : brokerPartitions) {
				...
				InputSplit split = new KafkaInputSplit(brokerId, zk.getBrokerName(brokerId), topic,
						Integer.valueOf(partitionId), lastConsumedOffset);
				splits.add(split);
			}
		}
		zk.close();
